angular.module('App').controller('results', ['$scope','srvc', 'catSrvc', '$window', '$document', function($scope, srvc, catSrvc, $window, $document){
    ...
    if( $window.mobilecheck() ){
        const viewPortHeight = $document[0].getElementsByClassName('contain-results')[0].offsetHeight;
        // console.log('viewPortHeight is...', viewPortHeight);
        $document[0].getElementsByClassName('contain-results')[0].setAttribute('style', `height:${viewPortHeight}px`);
    }
